...
def countOccurrences():
  ...
  for item in timeList:
    if item in occurrenceDictionary:
      occurrenceDictionary[item] += 1
    else:
      occurrenceDictionary[item] = 1
...
data = [go.Scatter(
            x=list(occurrenceDictionary.keys()), # keys go here
            y=list(occurrenceDictionary.values()))] # values go here
